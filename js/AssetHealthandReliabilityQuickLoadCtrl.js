app.controller('AssetHealthandReliabilityQuickLoadCtrl',['$rootScope','$scope','$http','$filter','$modal',function($rootScope,$scope,$http,$filter,$modal){
    ...
    $(this).addClass('active');
     ...
     $scope.UTCofCurrentDate.push(Date.UTC(year,month-1,1,1,1));
...
//ESN Dropdown and the Respective Chart
   ...
   $scope.display3="Hide";
    ...
    for(var i=0;i<response.data[0].length;i++){
        ...
        repairDateYear=$filter('date')(new Date(response.data[0][i].calendarDate),"yyyy");  
        repairDateMonth=$filter('date')(new Date(response.data[0][i].calendarDate),"M");
       
        repairDateMonth=repairDateMonth-1;
        repairDateDay=$filter('date')(new Date(response.data[0][i].calendarDate),"d");
        repairDateHour=0;
        repairDateMin=0;
...
$('#GetTableDataforParameter').hide();
   ...
   $scope.position="AtEventChart";
    ...
    for(var i=0;i<$scope.EventData.length;i++){
        ...
        repairDateYear=$filter('date')(new Date($scope.EventData[i].calendarDate),"yyyy");   
        repairDateMonth=$filter('date')(new Date($scope.EventData[i].calendarDate),"M");
        
        repairDateMonth=repairDateMonth-1;
        repairDateDay=$filter('date')(new Date($scope.EventData[i].calendarDate),"d");
      
        // if(repairDateYear.length==1){
        //    repairDateYear='200'+repairDateYear;
        // }
        
        // else{
        //     repairDateYear='20'+repairDateYear;
        // }
        // repairDateDay=01;
